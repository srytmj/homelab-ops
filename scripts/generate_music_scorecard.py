...
Clutter Files (.log/.url) : {ll['clutter_files']}       (100% Clean from logs/ads)
In-album M3U Playlists    : {ll['m3u_clutter']}       (100% Centralized Master Lossless.m3u8)
Duplicate Cover Art       : {ll['duplicate_covers']}       (100% Single official Cover.jpg per album)
Case Collisions (SMB/NTFS): {ll['case_collisions']}       (100% Safe for Windows clients)
Artists Missing Tilde (~) : {ll['missing_tilde']}       (100% of {ll['total_artists']:,} artists ending with ~)
Loose Audio in Roots      : {ll['loose_audio']}       (100% Located inside official album folders)
Empty Directories         : {ll['empty_dirs']}       (100% Clean from ghost folders)
...
FLAC Files in Lossy       : {ls['flac_files']}       (100% Clean, all FLAC rescued to Lossless)
Clutter Files in Lossy    : {ls['clutter_files']}       (0 .cue, 0 .log, 0 .htm)
Empty Dirs in Lossy       : {ls['empty_dirs']}       (0 Empty folders)
Case Collisions in Lossy  : {ls['case_collisions']}       (100% Safe for Windows clients)
...
def generate_markdown_doc(audit_data):
    ...
    status_text = "Zero-Defect Standard Achieved (0 Defects)" if defects == 0 else f"Defects Detected ({defects} defects)"
