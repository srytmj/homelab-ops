...
|---|---|---|---|
| Jellyfin | Movie/TV/anime streaming + music library (accessed via Feishin/foobar2000 as client, not Jellyfin web UI) | 8096 | `/mnt/hdd-music/jellyfin/music`, `/mnt/hdd-media/jellyfin/{movies,tv,anime}` — **deployed 2026-09-11, but these are currently placeholder directories on the internal SSD**, not the real external HDDs (storage hardware not physically assembled yet, see roadmap.md). Libraries will be empty until real media is added or the HDDs are mounted at these same paths. |
| Nextcloud | File sync/storage | 8080 | `/mnt/hdd-cloud/nextcloud` — **deployed 2026-09-11, placeholder directory on the internal SSD**, not the real external HDD yet. Uses the shared Postgres container (database `nextcloud`, same `admin` user/password as `postgres-redis.yml`) and shared Redis for caching. No admin account pre-created — first browser visit runs Nextcloud's own setup wizard (needs DB host `shared-postgres`, db `nextcloud`, user `admin`, password from `/opt/infra/nextcloud/.env` on the server). |
| Kavita | Manga/comic reader | 5000 | `/mnt/hdd-media/kavita/manga` — **deployed 2026-09-11, but this path is currently a placeholder directory on the internal SSD, not the real external HDD** (storage hardware not physically assembled yet, see roadmap.md). Library will be empty until real manga files are added, either now (temporary, on the SSD) or after the HDD is mounted at this same path. |
...
|---|---|---|---|
| Syncthing | Continuous P2P file sync between specific devices | 8384 (Web UI), 22000 tcp+udp, 21027 udp | Different from Nextcloud: no central "cloud" browsing/share-links, just keeps folders in sync across devices (including this server). **Deployed 2026-09-11.** Sync data at `/mnt/hdd-cloud/syncthing` — placeholder on internal SSD (real HDD not assembled yet). Hit a startup crash loop (`permission denied` on its config volume) because the image runs as UID 1000 by default but the Docker-created volume was owned by root — fixed with `chown -R 1000:1000` on the volume + config dir. **No login/auth configured** — the GUI is wide open on the LAN; worth setting a GUI username/password later via Settings. |
...
| Alexandrie | Self-hosted notes / knowledge base | - | Not yet deployed |
